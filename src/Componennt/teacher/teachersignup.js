...
export default function SignUp() {
  ...
  const onSubmit = async () => {
    ...
    const init = async () => {
      ...
      await db.collection("login_Teacher").doc(iid).set({
        Name: name,
        UserId: iid,
        Password: pass,
      });
      await db.collection("Teacher").doc(iid).set({
        UserId: iid,
        Name: name,
        Class: [],
        Schedule: [],
  ...
  return (
    <div className="spinner-border text-primary spinner-position pos1"></div>
  );
}
